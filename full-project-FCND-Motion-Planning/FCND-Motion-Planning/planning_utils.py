...
import numpy as np
sqrt_2 = 2**(float(1)/2)
def check_collinear(p1,p2,p3,eps=1e-4):
...
def remake_path(path):
    ...
    while i < len(new_path)-2:
        p1 = np.array([p(new_path[i])])
        p2 = np.array([p(new_path[i+1])])
        p3 = np.array([p(new_path[i+2])])
...
class Action(Enum):
    ...
    NORTH = (-1, 0, 1)
    SOUTH = (1, 0, 1)
    WEST_NORTH = (-1,-1,sqrt_2)
    WEST_SOUTH = (1,-1,sqrt_2)
    EAST_NORTH = (-1,1,sqrt_2)
    EAST_SOUTH = (1,1,sqrt_2)
    @property
    def cost(self):
        return self.value[2]
